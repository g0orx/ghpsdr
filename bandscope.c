...
static GtkWidget* bandscopeScrolledWindow;
static GtkAdjustment* horizontalAdjustment;
static GtkAdjustment* verticalAdjustment;
static GtkWidget* bandscope;
...
static int initBandscopeAverage=1;
static float averageBandscope[BANDSCOPE_BUFFER_SIZE];
float bandscopeAverageSmoothing=0.6f;


static int width;
static int height;
static GdkPixmap *bandscopePixmap=NULL;
static GdkPoint* bandscopePoints;
...
#ifdef INCLUDED
        switch(bandscopeZoom) {
            case 0:
                break;
            case 1:
                for(i=0;i<bandscopeWIDTH;i++) {
                    sample=(samples[i*2]+samples[(i*2)+1])/2.0F;
                    sample = sample + displayCalibrationOffset /*+ preampOffset*/;
                    bandscopePoints[i].x=i;
                    bandscopePoints[i].y=(int)(floor((bandscopeMAX - sample)*(float)bandscopeHEIGHT/yRange));
                }
                break;
            case 2:
                for(i=0;i<bandscopeWIDTH*2;i++) {
                    sample=samples[i];
                    sample = sample + displayCalibrationOffset /*+ preampOffset*/;
                    bandscopePoints[i].x=i;
                    bandscopePoints[i].y=(int)(floor((bandscopeMAX - sample)*(float)bandscopeHEIGHT/yRange));
